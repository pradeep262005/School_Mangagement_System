...
photo1=PhotoImage(file = r"H:\School\img\tr.png")
photoimage1=photo1.subsample(5,5)

# Entries Frame
entries_frame = Frame(root, bg="grey")
entries_frame.pack(side=TOP, fill=X)
title = Label(entries_frame, text="Amrita Vidyalayam Tirupur", font=("Georgia",32, "bold"), bg="grey", fg="#f39c12",image=photoimage,compound=RIGHT)
title.grid(row=0, columnspan=2, padx=10, pady=20, sticky="w")
title = Label(entries_frame, text="Teacher's Registartion", font=("Georgia",20, "bold"), bg="grey", fg="#f39c12",image=photoimage1,compound=RIGHT)
title.grid(row=1, columnspan=2, padx=10, pady=20, sticky="w")



lblName = Label(entries_frame, text="Name", font=("Forte", 16), bg="grey", fg="white")
...
lblAge = Label(entries_frame, text="Age", font=("Forte", 16), bg="grey", fg="white")
...
lbldoj = Label(entries_frame, text="D.O.B", font=("Forte", 16), bg="grey", fg="white")
...
lblsub = Label(entries_frame, text="Suject", font=("Forte", 16), bg="grey", fg="white")
lblsub.grid(row=3, column=2, padx=10, pady=10, sticky="w")
combosub= ttk.Combobox(entries_frame, font=("Corbel", 16), width=28, textvariable=sub, state="readonly")
combosub['values'] = ('Tamil','English','Maths','Science','CS','IP','ARTS','Social','Others')
combosub.grid(row=3, column=3, padx=10, sticky="w")

lblGender = Label(entries_frame, text="Gender", font=("Forte", 16), bg="grey", fg="white")
...
lblContact = Label(entries_frame, text="Contact No", font=("Forte", 16), bg="grey", fg="white")
...
lblaadhar = Label(entries_frame, text="AadharNumber", font=("Forte", 16), bg="grey", fg="white")
...
detailframe=Frame(root,bd=4,relief=RIDGE,bg='grey')
detailframe.place(x=0,y=680,width=1366,height=80)
lblsearch= Label(detailframe, text="Search", font=("Forte", 16), bg="grey", fg="white")
...
lblsearch1= Label(detailframe, text="Search", font=("Forte", 16), bg="grey", fg="white")
...
def dispalyAll():
    ...
    for row in dbt.fetch():
        tv.insert("", END, values=row)


def add_teacher():
    if txtName.get() == "" or txtAge.get() == "" or txtDoj.get() == "" or combosub.get() == "" or comboGender.get() == "" or txtContact.get() == "" or txtaadhar.get(
            1.0,END) == "":
        messagebox.showerror("Erorr in Input", "Please Fill All the Details")
        return
    dbt.insert(txtName.get(),txtAge.get(), txtDoj.get() , combosub.get() ,comboGender.get(), txtContact.get(), txtaadhar.get(
            1.0,END))
...
def update_teacher():
    if txtName.get() == "" or txtAge.get() == "" or txtDoj.get() == "" or combosub.get() == "" or comboGender.get() == "" or txtContact.get() == "" or txtaadhar.get(
        ...
        messagebox.showerror("Erorr in Input", "Please Fill All the Details")
        return
    dbt.update(row[0],txtName.get(), txtAge.get(), txtDoj.get(),combosub.get(), comboGender.get(), txtContact.get(),
              txtaadhar.get(
            1.0,END))
...
def SearchRecord():
    if search_txt.get() != "":
        ...
        cursor=conn.execute("SELECT * FROM teachers WHERE aadhar LIKE ?", ('%' + str(search_txt.get()) + '%',))
...
def SearchRecord1():
    if search_txt1.get() != "":
        tv.delete(*tv.get_children())
        conn = sqlite3.connect('teachers.db')
        cursor=conn.execute("SELECT * FROM teachers WHERE sub LIKE ?", ('%' + str(search_txt1.get()) + '%',))
        fetch = cursor.fetchall()
        for row in fetch:
            tv.insert('', 'end', values=row)
        cursor.close()
...
btnAdd = Button(btn_frame, command=add_teacher, text="ADD", width=15, font=("Forte", 16, "bold"), fg="white",
                bg="#16a085", bd=0).grid(row=0, column=0)
btnEdit = Button(btn_frame, command=update_teacher, text="UPDATE", width=15, font=("Forte", 16, "bold"),
                 fg="white", bg="#2980b9",
                 bd=0).grid(row=0, column=1, padx=10)
btnDelete = Button(btn_frame, command=delete_teacher, text="DELETE", width=15, font=("Forte", 16, "bold"),
                   fg="white", bg="#c0392b",
                   bd=0).grid(row=0, column=2, padx=10)
btnClear = Button(btn_frame, command=clearAll, text="CLEAR", width=15, font=("Forte", 16, "bold"), fg="white",
                  bg="#f39c12",
...
tree_frame.place(x=0, y=445, width=1366, height=240)
style = ttk.Style()
